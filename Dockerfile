ARG ARCH
FROM docker.io/alberic89/wesnoth-ci:${ARCH}-alpine-latest
WORKDIR /home/wesnoth-travis/wesnoth/
RUN cmake . -DCMAKE_BUILD_TYPE=Release -DFORCE_COLOR_OUTPUT=true -DENABLE_LTO=true -DLTO_JOBS=`nproc` && make -j`nproc` && make install -j`nproc` 
